...
use renderers::webgl::{WebGL, KeyboardListenerWGL, MouseListenerWGL};
use crate::{engine::{camera::{PerspectiveCamera}, input::{keyboard::{KeyboardKey, KeyboardListener}, mouse::MouseListener}, objectg::ObjectG, scene::{Scene}, script::Script, clock::Clock}, extra::{color::Color, wasm_mappings::{map_panic}}, graph::{mesh::{CUBE_INDICES, CUBE_VERTICES, MeshPrimitives}, renderer::Renderer, window::Window}, math::quaternion::Quaternion32};
...
pub fn main() {
    ...
    let clock = Clock::new();
    let mut keyboard_listener = KeyboardListenerWGL::new();
    let mut mouse_listener = MouseListenerWGL::new();
    renderer.run(scene);
}
